import requests
import pandas as pd
from top50 import top50_files
from top50 import affiliation_unification
from top50 import affiliations_to_id_dict
...
def get_affiliations(df):
    ...
    for i in df.index:
        place = df.loc[i, 'Место'].upper()
        for affil in affiliations_to_id_dict.keys():
            if affil.upper() in place:
...
# Получение общего количества публикаций для данного института для данного квартиля для данного года для всех источников этого квартиля
def get_publications_amount_in_quartile(q_cur, affil, year, batch_size, api_keys_index):
    start = 0
    end = batch_size
    ...
    while (start != max_sources_number):
        query = get_query_for_quartiles(q_cur, start, end, affil, year)
...
def get_query_for_quartiles(q_cur, start, end, affil, year):
    query = '?query=('

    for id in q_cur[start:end]:
        query += 'SOURCE-ID({0}) OR '.format(id)

    query = query[:-4] + ') AND AF-ID({0})'.format(affiliations_to_id_dict[affil])
...
def main():
    ...
    for file in top50_files[7:8]:
        year = int(file[6:10])      # Получение года из навзвания файла редакции Top50
        ...
        affiliations = get_affiliations(df)

        df = pd.DataFrame(columns=['Affiliation', 'Year', 'Q1', 'Q2', 'Q3', 'Q4', 'Top10'])
        index_in_df = 0

        print('Year {0}, list {1}'.format(year, file))   # Вывод прогресса программы в консоль
        print(affiliations)                              # Всего уникальных интститутов в редакции

        for affil in affiliations[9:10]:
            quartile_index = 0

            for q_cur in qs_list:
                quartile_index += 1
                print('Quartile {0} in {1}'.format(quartiles_conformity[quartile_index], affil)) # Вывод прогресса программы в консоль

                publications_amount, api_keys_index = get_publications_amount_in_quartile(q_cur, affil, year, batch_size, api_keys_index)
                df.loc[index_in_df, quartiles_conformity[quartile_index]] = publications_amount

            print('Top10 in {0}'.format(affil))           # Вывод прогресса программы в консоль
            publications_amount, api_keys_index = get_publications_amount_in_quartile(top10, affil, year, batch_size, api_keys_index)
            df.loc[index_in_df, 'Top10'] = publications_amount

            df.loc[index_in_df, 'Affiliation'] = affil
